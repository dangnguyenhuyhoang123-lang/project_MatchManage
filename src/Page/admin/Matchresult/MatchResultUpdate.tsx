...
type EditableMatchStats = Omit<MatchStatsUpsertRequest, "teamId">;

const EMPTY_EDITABLE_STATS: EditableMatchStats = {
  possession: 0,
  shots: 0,
  shotsOnTarget: 0,
  corners: 0,
...
function statsToEditable(stats?: MatchStats | null): EditableMatchStats {
  return {
    ...
    totalPasses: Number(stats?.totalPasses ?? 0),
...
function playerSeasonToLineupOption(player: PlayerSeason): MatchLineup {
  ...
  return {
    id: Number(player.id ?? playerId),
    playerId,
    playerName: player.playerName || `Cầu thủ #${playerId}`,
...
const MatchResultUpdate: FC<MatchResultUpdateProps> = ({
  ...
  const [eventPlayersByTeam, setEventPlayersByTeam] = useState<
    Record<number, MatchLineup[]>
  >({});
  ...
  const loadEventPlayersByTeam = async (
    match: typeof currentMatch,
  ): Promise<Record<number, MatchLineup[]>> => {
    const teamIds = [match?.homeTeam?.id, match?.awayTeam?.id].filter(
      (teamId): teamId is number => Number.isFinite(Number(teamId)),
    );

    const result: Record<number, MatchLineup[]> = {};
    ...
    await Promise.all(
      teamIds.map(async (teamId) => {
        try {
          ...
          const playerSeasonRes =
            await PlayerSeasonService.getPlayerSeasonsByTeamSeason(
              teamSeasonId,
            );

          result[teamId] = readArray<PlayerSeason>(playerSeasonRes.data)
            .filter((player) => Number.isFinite(Number(player.playerId)))
            .map(playerSeasonToLineupOption);
        } catch (error) {
          console.error(
            "Cannot load registered players for match event",
            error,
          );
          result[teamId] = [];
        }
  ...
  const reloadMatchData = async () => {
    ...
    const eventPlayers = await loadEventPlayersByTeam(updatedMatch);
    ...
    const nextAwayStats = (statsData || []).find(
      (item) => item.teamId === updatedMatch?.awayTeam?.id,
    );
    setHomeStatsDraft(statsToEditable(nextHomeStats));
    setAwayStatsDraft(statsToEditable(nextAwayStats));

    setLineups(lineupsData);
    setEventPlayersByTeam(eventPlayers);
  ...
  const handleStatsDraftChange = (
    side: "home" | "away",
    field: keyof EditableMatchStats,
    value: string,
  ) => {
    const numericValue = Math.max(0, Number(value) || 0);
    const updater = (current: EditableMatchStats): EditableMatchStats => ({
      ...current,
      [field]: numericValue,
    });

    if (side === "home") {
      setHomeStatsDraft(updater);
    } else {
      setAwayStatsDraft(updater);
    }
  };
  ...
  const handleSaveStats = async () => {
    ...
    try {
      const payload: MatchStatsUpsertRequest[] = [
        { teamId: homeTeamId, ...homeStatsDraft },
                ...
                <section className="bg-white rounded-2xl p-6 border border-black/5 shadow-sm">
                  <div className="mb-5 flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="rounded-full bg-green-800 px-3 py-1.5 text-[11px] font-black text-white disabled:opacity-50"
                    >
                      Lưu
                    </button>
                  </div>

                  <div className="mb-4 grid grid-cols-[1fr_72px_72px] items-center gap-2 text-[10px] font-black uppercase tracking-wider text-gray-400">
                    <span>Chỉ số</span>
                    <span className="text-center">Nhà</span>
                  ...
                  <div className="space-y-2">
                    <StatsInputRow
                      label="Kiểm soát %"
                      field="possession"
                      homeValue={homeStatsDraft.possession}
                      awayValue={awayStatsDraft.possession}
                      onChange={handleStatsDraftChange}
                    />
                    <StatsInputRow
                      label="Sút bóng"
                      field="shots"
                      homeValue={homeStatsDraft.shots}
                      awayValue={awayStatsDraft.shots}
                      onChange={handleStatsDraftChange}
                    />
                    <StatsInputRow
                      label="Trúng đích"
                      field="shotsOnTarget"
                      homeValue={homeStatsDraft.shotsOnTarget}
                      awayValue={awayStatsDraft.shotsOnTarget}
                      onChange={handleStatsDraftChange}
                    />
                    <StatsInputRow
                      label="Phạt góc"
                      field="corners"
                      homeValue={homeStatsDraft.corners}
                      awayValue={awayStatsDraft.corners}
                      onChange={handleStatsDraftChange}
                    />
                    <StatsInputRow
                      label="Phạm lỗi"
                      field="fouls"
                      homeValue={homeStatsDraft.fouls}
                      awayValue={awayStatsDraft.fouls}
                      onChange={handleStatsDraftChange}
                    />
                    <StatsInputRow
                      label="Việt vị"
                      field="offsides"
                      homeValue={homeStatsDraft.offsides}
                      awayValue={awayStatsDraft.offsides}
                      onChange={handleStatsDraftChange}
                    />
                    <StatsInputRow
                      label="Thẻ vàng"
                      field="yellowCards"
                      homeValue={homeStatsDraft.yellowCards}
                      awayValue={awayStatsDraft.yellowCards}
                      onChange={handleStatsDraftChange}
                    />
                    <StatsInputRow
                      label="Thẻ đỏ"
                      field="redCards"
                      homeValue={homeStatsDraft.redCards}
                      awayValue={awayStatsDraft.redCards}
                      onChange={handleStatsDraftChange}
                    />
                    <StatsInputRow
                      label="Đường chuyền"
                      field="totalPasses"
                      homeValue={homeStatsDraft.totalPasses}
                      awayValue={awayStatsDraft.totalPasses}
                      onChange={handleStatsDraftChange}
                    />
                    <StatsInputRow
                      label="Chính xác %"
                      field="passAccuracy"
                      homeValue={homeStatsDraft.passAccuracy}
                      awayValue={awayStatsDraft.passAccuracy}
                      onChange={handleStatsDraftChange}
                    />
                  </div>
                </section>
...
const StatsInputRow = ({
  ...
  <div className="grid grid-cols-[1fr_72px_72px] items-center gap-2 rounded-xl bg-[#f5f3ef] px-3 py-2">
    <span className="text-[11px] font-bold text-gray-500">{label}</span>
    <input
      type="number"
      min={0}
      value={homeValue}
      onChange={(event) => onChange("home", field, event.target.value)}
      className="w-full rounded-lg bg-white px-2 py-1.5 text-center text-sm font-black text-green-700 outline-none focus:ring-2 focus:ring-green-700/20"
    />
    <input
      type="number"
      min={0}
