...
const PlayerRegistration: React.FC<Props> = ({
  ...
  useEffect(() => {
    const fetchTeamPlayers = async () => {
      try {
        ...
        const response = await PlayerService.getAllPlayersNormalized(0, 100, { teamId: 1 });
  ...
  const handleConfirmSelection = () => {
    const playersToAdd = availablePlayers.filter(p => selectedPlayerIds.includes(p.id));
    if (addMode === "main") {
  ...
  const allCurrentIds = new Set([...mainPlayers.map(p => p.id), ...subPlayers.map(p => p.id)]);
  const unselectedAvailablePlayers = availablePlayers.filter(p => !allCurrentIds.has(p.id));
      ...
      <div className="bg-[#fff9c4]/40 border border-[#fbc02d]/30 p-5 rounded-xl mb-8 flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-[#f57f17]/80 text-xs mt-1">
            Danh sách hiện có <span className="font-bold">{totalSelected}/18</span> cầu thủ.
            {totalSelected < 14 && (
            ...
            {mainPlayers.map((player) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <img
                    src={player.avatar || `https://placehold.co/100x100?text=${player.position || "Player"}`}
                    alt={player.name}
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <span className={`text-[10px] px-2 py-0.5 font-bold rounded bg-blue-100 text-blue-700`}>
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-[#4c56af] font-bold uppercase">
                      {player.position || "Cầu thủ"} • {calculateAge(player.dateOfBirth)} tuổi
                    </p>
                  </div>
          ...
          <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            <ul className="space-y-5">
              <li className="flex items-start gap-3">
                <span className={`material-symbols-outlined ${totalSelected >= 14 ? "text-[#0d631b]" : "text-red-500"}`}>
                  {totalSelected >= 14 ? "check_circle" : "cancel"}
                </span>
                <div>
                  <p className="text-sm font-bold">Số lượng tối thiểu</p>
                  <p className="text-[11px] text-gray-400">{totalSelected}/18 cầu thủ (cần tối thiểu 14)</p>
                </div>
              </li>
            ...
            <div className="mt-8 pt-6 border-t border-gray-100">
              ...
              <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full bg-[#0d631b] transition-all" style={{ width: `${Math.min((totalSelected / 18) * 100, 100)}%` }} />
              </div>
            </div>
          </section>
        </div>
              ...
              unselectedAvailablePlayers.map(player => (
                <div 
                  ...
                  className={`flex items-center gap-4 p-3 rounded-xl border cursor-pointer transition-all ${
                    selectedPlayerIds.includes(player.id) ? "border-green-500 bg-green-50" : "border-gray-100 hover:border-green-300"
                  }`}
                >
                  <div className={`w-5 h-5 rounded-md flex items-center justify-center border ${
                    selectedPlayerIds.includes(player.id) ? "bg-green-500 border-green-500" : "border-gray-300 bg-white"
                  }`}>
                    {selectedPlayerIds.includes(player.id) && <span className="material-symbols-outlined text-white text-xs font-bold">check</span>}
                  </div>
                  ...
                  <div className="flex-1">
                    <h4 className="font-bold text-gray-900 text-sm">{player.name}</h4>
                    <p className="text-xs text-gray-500">{player.position || "Cầu thủ"} • {calculateAge(player.dateOfBirth)} tuổi</p>
                  </div>
                </div>
              ))
            )}
