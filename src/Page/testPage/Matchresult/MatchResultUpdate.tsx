...
const MatchResultUpdate: FC<MatchResultUpdateProps> = ({ matchData, onClose, onUpdateSuccess }) => {
  ...
  // Giữ lại mock data cho phần UI timeline & stats để giao diện không bị trống
  const [goals, setGoals] = useState<GoalEvent[]>([
    {
      id: "1",
      player: "Cầu thủ ghi bàn",
      minute: 23,
  ...
  const handleUpdate = async () => {
    ...
    setLoading(true);
    try {
      // Chuẩn bị payload theo format của MatchModel (bỏ qua những phần phức tạp nếu BE chỉ cần field cơ bản)
      const updatedMatch = {
        ...matchData,
        status: "FINISHED", // Cập nhật trạng thái thành đã kết thúc
        homeScore: homeScore,
        awayScore: awayScore,
      };
            ...
            <div className="flex flex-col items-center gap-4 z-10">
              <div className="flex items-center gap-4 md:gap-6">
                <input
                  ...
                  className="w-16 md:w-20 text-center text-4xl md:text-5xl font-black bg-gray-100 border-2 border-transparent focus:border-green-500 rounded-xl p-2 outline-none transition-colors"
                />
                <span className="text-gray-300 text-3xl md:text-4xl font-bold">:</span>
                <input
                  type="number"
                  min="0"
                  value={awayScore}
            ...
            <div className="col-span-12 md:col-span-9 space-y-6">
              <section className="bg-[#f5f3ef] rounded-2xl p-6">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-lg font-bold text-green-900 flex items-center gap-2">
                    ...
                    Danh sách bàn thắng (Minh họa)
                  </h3>
                  <button className="text-green-700 font-bold text-sm flex items-center gap-1 opacity-50 cursor-not-allowed" title="Tính năng đang phát triển">
                ...
                <div className="space-y-3">
                  ...
                  {goals.length === 0 && (
                    <p className="text-sm text-gray-500 text-center py-4">Chưa có bàn thắng nào được ghi nhận.</p>
                  )}
                </div>
              </section>
              ...
              <section className="bg-[#f5f3ef] rounded-2xl p-6 hidden md:block">
                <h3 className="text-lg font-bold text-green-900 mb-8 flex items-center gap-2">
                  ...
                  Timeline trận đấu trực quan (Minh họa)
                </h3>
                <div className="relative pt-12 pb-16 px-4">
                  <div className="h-1.5 w-full bg-gray-300 rounded-full"></div>
                  <TimelineMarker minute={23} label="Goal" type="goal" team="home" />
                  <TimelineMarker minute={32} label="Thẻ vàng" type="card" color="bg-yellow-400" />
                  <TimelineMarker minute={45} label="Goal" type="goal" team="away" position="50%" />
                  <TimelineMarker minute={88} label="Thẻ đỏ" type="card" color="bg-red-500" position="90%" />
                </div>
              </section>
            </div>
            ...
            <div className="col-span-12 md:col-span-3 space-y-6">
              <section className="bg-white rounded-2xl p-6 border border-black/5 shadow-sm">
                ...
                <div className="space-y-6">
                  <StatBar label="Kiểm soát" homeVal={50} awayVal={50} />
                  <StatRow label="Sút bóng" homeVal={10} awayVal={10} />
                  <StatRow label="Phạm lỗi" homeVal={8} awayVal={7} />
                  <StatRow label="Phạt góc" homeVal={4} awayVal={5} last />
                </div>
              </section>
  ...
  <div className="flex flex-col items-center gap-3 w-1/3 z-10">
    <div className="w-16 h-16 md:w-24 md:h-24 bg-white rounded-full flex items-center justify-center shadow-md border-4 border-gray-50 overflow-hidden">
      {logo ? (
        ...
        <span className="material-symbols-outlined text-gray-400 text-3xl">shield</span>
      )}
    </div>
    <span className="text-base md:text-xl font-black text-green-900 font-['Be_Vietnam_Pro'] text-center px-2 line-clamp-2">
      {name}
    </span>
    <span className={`${color} px-3 py-1 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-wider`}>
      {role}
    </span>
  </div>
);

const GoalRow = ({ goal, onDelete }: { goal: GoalEvent; onDelete: (id: string) => void }) => (
  <div className={`bg-white p-3 md:p-4 rounded-xl flex items-center gap-3 md:gap-4 shadow-sm ${goal.team === "away" ? "border-l-4 border-indigo-500" : "border-l-4 border-green-500"}`}>
    <div className={`w-8 h-8 md:w-10 md:h-10 shrink-0 ${goal.team === "home" ? "bg-green-100 text-green-700" : "bg-indigo-100 text-indigo-700"} flex items-center justify-center rounded-full`}>
      <span className="material-symbols-outlined fill-1 text-[18px] md:text-[24px]">sports_soccer</span>
    </div>
    <div className="flex-1 grid grid-cols-3 gap-2 md:gap-4">
      <div>
        <p className="text-[9px] md:text-[10px] font-bold text-gray-400 uppercase mb-0.5">Cầu thủ</p>
        <p className="text-xs md:text-sm font-bold truncate">{goal.player}</p>
      </div>
      <div>
        <p className="text-[9px] md:text-[10px] font-bold text-gray-400 uppercase mb-0.5">Thời điểm</p>
        <p className="text-xs md:text-sm font-bold">{goal.minute}'</p>
      </div>
      <div>
        <p className="text-[9px] md:text-[10px] font-bold text-gray-400 uppercase mb-0.5">Loại</p>
        <p className="text-xs md:text-sm font-bold truncate">{goal.type}</p>
      </div>
    </div>
    <button type="button" onClick={() => onDelete(goal.id)} className="text-gray-300 hover:text-red-500 shrink-0 p-1">
      <span className="material-symbols-outlined text-[20px]">delete</span>
    </button>
  </div>
);

const TimelineMarker = ({ minute, label, type, color = "bg-green-600", team, position }: any) => {
  const leftPos = position || `${(minute / 90) * 100}%`;
  return (
    <div className="absolute top-12 -translate-x-1/2 flex flex-col items-center" style={{ left: leftPos }}>
      <div className={`w-0.5 ${type === "goal" ? "h-10" : "h-6"} ${team === "away" ? "bg-indigo-500" : "bg-green-600"} mb-1`}></div>
      {type === "goal" ? (
        <div className={`w-8 h-8 rounded-full ${team === "away" ? "bg-indigo-500" : "bg-green-600"} flex items-center justify-center text-white shadow-lg`}>
          <span className="material-symbols-outlined text-xs fill-1">sports_soccer</span>
        </div>
      ) : (
        <div className={`w-5 h-7 rounded-sm ${color} border border-gray-300 shadow-sm`}></div>
      )}
      <span className="text-[10px] font-bold mt-1 whitespace-nowrap">{label} {minute}'</span>
    </div>
  );
};
...
const StatRow = ({ label, homeVal, awayVal, last = false }: any) => (
  <div className={`flex items-center justify-between py-1.5 ${last ? "" : "border-b border-gray-50"}`}>
    <span className="text-lg font-black text-green-700 w-8 text-center">{homeVal}</span>
    <span className="text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-tighter flex-1 text-center">
      {label}
    </span>
    <span className="text-lg font-black text-indigo-600 w-8 text-center">{awayVal}</span>
  </div>
);
